...
print("video data loaded with shape:", audio_data.shape)
...
def predict_datapoint(audio, video, target):
    ...
    last_pred = valence_model.predict([audio_last, video_last]).reshape(-1)
...
for subject in sbj_n:
	for story in str_n:
		...
		annotations = np.loadtxt(annotations_path + name + '.csv')
		num_frames = len(annotations, skiprows=1)
		...
		print '{} ccc {}'.format(name, ccc2(label_slice, predictions))
		start = end
